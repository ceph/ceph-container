...
function build_ceph_imgs {
  ...
  for ceph_release in "${CEPH_RELEASES[@]}"; do
    ...
    make BASEOS_TAG=stream"${CENTOS_RELEASE}" DAEMON_BASE_TAG=base:"$RELEASE"-"${ceph_release}"-centos-stream"${CENTOS_RELEASE}"-aarch64 DAEMON_TAG=daemon:"$RELEASE"-"${ceph_release}"-centos-stream"${CENTOS_RELEASE}"-aarch64 RELEASE="${RELEASE}" FLAVORS="${ceph_release},centos-arm64,${CENTOS_RELEASE}" BASEOS_REGISTRY="${CONTAINER_REPO_HOSTNAME}/centos" BASEOS_REPO=centos TAG_REGISTRY="${CONTAINER_REPO_ORGANIZATION}" build
  done
  docker images
}
