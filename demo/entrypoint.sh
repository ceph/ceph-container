...
if [ ! -e /var/lib/ceph/mon/${CLUSTER}-${MON_NAME}/keyring ]; then
   ...
   if [ ! -e /etc/ceph/${CLUSTER}.mon.keyring ]; then
      echo "ERROR- /etc/ceph/${CLUSTER}.mon.keyring must exist.  You can extract it from your current monitor by running 'ceph --cluster ${CLUSTER} auth get mon. -o /tmp/${CLUSTER}.mon.keyring'"
      exit 3
   fi

   if [ ! -e /etc/ceph/${CLUSTER}.monmap ]; then
      echo "ERROR- /etc/ceph/${CLUSTER}.monmap must exist.  You can extract it from your current monitor by running 'ceph --cluster ${CLUSTER} mon getmap -o /tmp/monmap'"
      exit 4
   fi
   ...
   ceph-mon --mkfs -i ${MON_NAME} --monmap /etc/ceph/${CLUSTER}.monmap --keyring /tmp/${CLUSTER}.mon.keyring
...
# start MON
ceph-mon -i ${MON_NAME} --public-addr ${MON_IP}

# change replica size
ceph --cluster ${CLUSTER} osd pool set rbd size 1
...
if [ ! -e /var/lib/ceph/osd/${CLUSTER}-0/keyring ]; then
  ...
  ceph-osd --cluster ${CLUSTER} -i 0 --mkfs
  ceph --cluster ${CLUSTER} auth get-or-create osd.0 osd 'allow *' mon 'allow profile osd' -o /var/lib/ceph/osd/${CLUSTER}-0/keyring
  ceph --cluster ${CLUSTER} osd crush add 0 1 root=default host=$(hostname -s)
  ceph-osd --cluster ${CLUSTER} -i 0 -k /var/lib/ceph/osd/${CLUSTER}-0/keyring
fi
...
if [ ! -e /var/lib/ceph/mds/${CLUSTER}-0/keyring ]; then
  ...
  ceph --cluster ${CLUSTER} osd pool create cephfs_data 8
  ceph --cluster ${CLUSTER} osd pool create cephfs_metadata 8
  ceph --cluster ${CLUSTER} fs new cephfs cephfs_metadata cephfs_data

  # bootstrap MDS
  mkdir -p /var/lib/ceph/mds/${CLUSTER}-0
  ceph --cluster ${CLUSTER} auth get-or-create mds.0 mds 'allow' osd 'allow *' mon 'allow profile mds' > /var/lib/ceph/mds/${CLUSTER}-0/keyring
fi
...
if [ ! -e /var/lib/ceph/radosgw/${RGW_NAME}/keyring ]; then
  ...
  ceph --cluster ${CLUSTER} auth get-or-create client.radosgw.gateway osd 'allow rwx' mon 'allow rw' -o /var/lib/ceph/radosgw/${RGW_NAME}/keyring
fi
...
# start ceph-rest-api
ceph-rest-api --cluster ${CLUSTER} -n client.admin &
