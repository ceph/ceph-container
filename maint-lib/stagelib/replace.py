...
# Support __VARIABLE__ file replacement for variables
# matching '__<VARIABLE_NAME>__'.
...
# '__ENV_[<GLOBAL_VAR_NAME>]__'. Support only globals w/ capital
...
def _get_file_text(file_path):
    try:
        ...
        IOOSErrorGracefulFail(o, "Cannot read "
                                 "or decode {} \n".format(file_path))
    ...
    try:
        with open(variable_file_path) as variable_file:
            ...
            if "\n" in variable_text or len(variable_text) > 46:
                ...
                logging.info(REPLACE_LOGTEXT.format(variable,
                                                    '[user-defined script]',
                                                    file_path))
            else:
                logging.info(REPLACE_LOGTEXT.format(variable,
...
def _save_with_backup(file_path, text):
    try:
        ...
        IOOSErrorGracefulFail(o, "Could not rename file "
                                 "{0} to {0}.bak".format(file_path))
...
def _file_replace(template_text, file_path, variable_file_dir):
    ...
    for var in variable_matches:
        template_text = _replace_file_in_text(os.path.join(variable_file_dir,
                                                           var),
...
def _env_var_replace(template_text, file_path):
    ...
    for var_name in variable_matches:
        ...
        try:
            ...
            sys.stderr.write(
                'Variable {} in {} could not be replaced, '
                ' because the env var {} is unset'.format(
                    text_to_replace, file_path, var_name))
            sys.exit(1)
        logging.info(REPLACE_LOGTEXT.format(text_to_replace,
                                            var_value,
                                            file_path))
...
# Given a file path, do file and env replacements on it repeatedly until no
...
def _do_replace_on_file(file_path, replace_root_dir):
    ...
    while True:
        ...
        env_replacements, rendered_text = _env_var_replace(rendered_text,
                                                           file_path)
...
def do_variable_replace(replace_root_dir):
    ...
    For all files recursively in the replace root dir, do 2 things:
     1. For each __VARIABLE__ in the File, look for a corresponding
        __VARIABLE__ file in the replace root dir, and replace the
        __VARIABLE__ in the File with the text from the corresponding
        __VARIABLE__ file.
     2. For each __ENV_[<ENV_VAR>]__ variable in the File,
        replace the variable in the File with the content of the
    ...
    for dirname, subdirs, files in os.walk(replace_root_dir, topdown=True):
        for f in files:
            ...
            total_replacements, \
                rendered_text = _do_replace_on_file(file_path,
                                                    replace_root_dir)
