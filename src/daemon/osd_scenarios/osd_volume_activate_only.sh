...
function osd_volume_activate_only {
  : "${OSD_ID:?Give me an OSD ID to activate only, eg: -e OSD_ID=0}"
  ...
  function sigterm_cleanup_post {
    ...
    for mnt in $ceph_mnt; do
      log "osd_volume_activate_only: Unmounting $mnt"
      umount "$mnt" || (log "osd_volume_activate_only: Failed to umount $mnt"; lsof "$mnt")
    done
    ...
    for uuid in ${UUIDS}; do
      ...
      if echo "${DATA}" | grep -qo "${uuid}"; then
        log "osd_volume_activate_only: Closing dmcrypt $uuid"
        cryptsetup close "${uuid}" || log "osd_volume_activate_only: Failed to close dmcrypt ${uuid}"
      fi
    done
  }
